...
function processDrpdown($selectedVal) {
    ...
    if($q == mar){
      $conn=mysqli_connect('sophia.cs.hku.hk','ywlai','YZjNGaaa') or die ('Failed to Connect '.mysqli_error($conn));
      mysqli_select_db($conn, 'ywlai') or die ('Failed to Access DB'.mysqli_error($conn));
      $query = "SELECT SUM(Sales) from Sunposales WHERE Date >= '2020-03-01' AND Date <= '2020-03-30' ";
      $exec = mysqli_query($conn,$query);
      while($row = mysqli_fetch_array($exec)){
      echo "".$row['SUM(Sales)']."";
         }
    ...
    if($q == apr){
      $conn=mysqli_connect('sophia.cs.hku.hk','ywlai','YZjNGaaa') or die ('Failed to Connect '.mysqli_error($conn));
      mysqli_select_db($conn, 'ywlai') or die ('Failed to Access DB'.mysqli_error($conn));
      $query = "SELECT SUM(Sales) from Sunposales WHERE Date >= '2020-04-01' AND Date <= '2020-04-31' ";
      $exec = mysqli_query($conn,$query);
      while($row = mysqli_fetch_array($exec)){
      echo "".$row['SUM(Sales)']."";
         }
