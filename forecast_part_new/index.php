...
var options = {  title: '', vAxis: {title: 'HKD($)', titleTextStyle :{fontSize : 18, italic:false } }};
...
$(document).ready(function(){
   $('#selectweek_feb').change(function(){
           ...
           $.post('getsales.php', { dropdownValue: inputValue },function(data1){
            document.getElementById("total").innerHTML = "$" + data1;
           });
           $.post('getaverage.php', { dropdownValue: inputValue },function(data2){
            document.getElementById("average").innerHTML = "$" + data2;
           });
...
$(document).ready(function(){
  $('#selectmonth').change(function(){
    ...
    document.getElementById("selectweek_feb").value = "";
    document.getElementById("selectweek_mar").value = "";
    document.getElementById("selectweek_apr").value = "";
    document.getElementById("selectmonth").value = "";
});
});
...
function getingredTable(){
  ...
  $('#ingredTable').append('<tr><th style="width:20%;text-align:center;">食材</th>'+
    ...
    '<th style="width:20%;text-align:center;">現有存量</th><th style="width:20%;text-align:center;">建議入貨日期</th></tr>'
  );
...
function modifyinventory(item){
  $v = document.getElementById("item"+item).value;
...
function loadtotal(){
  ...
  <?php
   ...
   mysqli_select_db($conn, 'ywlai') or die ('Failed to Access DB'.mysqli_error($conn));
     ...
     $query = "SELECT SUM(Sales) from Sunposales WHERE Date >= '2020-02-01' AND Date <= '2020-04-30' ";
     $exec = mysqli_query($conn,$query);
     while($row = mysqli_fetch_array($exec)){
     echo "[".$row['SUM(Sales)']."]";
      }
...
function loadaverage(){
  ...
  <?php
   ...
   mysqli_select_db($conn, 'ywlai') or die ('Failed to Access DB'.mysqli_error($conn));
     ...
     $query = "SELECT AVG(Sales) from Sunposales WHERE Date >= '2020-02-01' AND Date <= '2020-04-30' ";
     $exec = mysqli_query($conn,$query);
     while($row = mysqli_fetch_array($exec)){
